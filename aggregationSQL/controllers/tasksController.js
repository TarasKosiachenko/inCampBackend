...
class TasksController {
    async get(req, res) {
        ...
        (list_id) ? tasksModel.getTasksByListId(list_id).then(tasks => res.json(tasks))
    ...
    async createTask(req, res) {
        ...
        (list_id) ? tasksModel.createTask(list_id, name, description, done, due_date).then(task => res.json(task))
    ...
    async replacingTask(req, res) {
        ...
        tasksModel.replacingTask(task_id, list_id, name, description, done, due_date).then(task => res.json(task));
    }; 
    ...
    async deleteTask(req, res) {
        const task_id = parseInt(req.params.id);
        tasksModel.deleteTask(task_id).then(task => res.json('Task has been deleted'));
    }
}
